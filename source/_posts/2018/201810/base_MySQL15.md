...
#### Server Section
    ```sql
        ...
        basedir="C:/Program Files/MySQL/Mysql server 5.7/" # 基准路径,其他路径都相对于这个路径

        datadir="C:/Program Files/MySQL/MySQL Server 5.7/Data" # mysql数据库文件所在目录
        ...
        max_connections=512 # mysql服务器支持的最大并发连接数（用户数）.但总会预留其中的一个连接给管理员使用超级权限登录,即使连接数目达到最大限制.如果设置得过小而用户比较多,会经常出现“Too many connections”错误.
