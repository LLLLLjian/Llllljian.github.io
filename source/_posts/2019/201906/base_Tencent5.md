...
#### 已经实现的功能
    将用户发送给公众号的信息存到mongo中并重写了_id, 实现了自增

#### Index.php
- 测试用
    ```bash
        [llllljian@llllljian-cloud-tencent controller 17:56:12 #4]$ cat Index.php
        <?php
        ...
        class Index extends Controller
        {
            ...
            {
                $res = Db::connect("db_mongo")->name("phone_log")->where("mobile", "13978412182")->select();
                dump($res);
            }
        }
    ```
...
- 与公众号平台交互
    ```bash
        [llllljian@llllljian-cloud-tencent controller 17:57:23 #12]$ cat Wechat.php
        <?php
        /**
        ...
        **/
        ...
        class Wechat extends Controller
        {
            ...
            protected function getPhoneInfo($mobile, $sqlType = 1)
            {
                $res = array();
                if ($sqlType == 2) {
                    $res = Db::connect("db_mongo")->name("phone_log")->where("mobile", $mobile)->find();
                } else {
                    $res = "";
                }
