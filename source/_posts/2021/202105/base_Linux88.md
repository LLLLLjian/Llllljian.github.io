...
- 是什么
    * /etc/init.d里需要用到的函数
- 能干什么
    * 启动、停止或重新启动Linux和UNIX系统守护进程
- code
    ```bash
        # -*-Shell-script-*-
        #
        # 该脚本几乎被 /etc/init.d/ 下的所有脚本所调用, 因为它包含了大量的的基础函数.同时也被 /etc/rc.d/rc.sysinit , 例如 success、action、failure 等函数
        ...
        # 检查是否已存在pid, 如果有一个存在, 返回0(通过查看/proc目录)
        checkpid() {
            local i
        ...
        # 启动某个服务./etc/init.d目录部分脚本的start使用到这个
        daemon() {
            # Test syntax.
        ...
        # 杀死某个进程./etc/init.d目录部分脚本的stop使用到这个
        killproc() {
